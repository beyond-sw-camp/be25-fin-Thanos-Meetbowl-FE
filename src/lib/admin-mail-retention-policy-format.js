...
export function toRetentionDays(years, months, days) {
...
export function fromRetentionDays(retentionDays) {
  ...
  let days = remainingDays - (months * DAYS_PER_MONTH)
  ...
  return {
    ...
    days: Math.min(Math.max(days, 0), MAX_DAYS),
  }
}

export function validateRetentionPeriod(years, months, days) {
  const numericYears = Number(years)
  const numericMonths = Number(months)
  const numericDays = Number(days)

  if (!Number.isInteger(numericYears) || !Number.isInteger(numericMonths) || !Number.isInteger(numericDays)) {
  ...
  const retentionDays = toRetentionDays(numericYears, numericMonths, numericDays)
...
export function formatRetentionPeriod(years, months, days) {
  // helper text와 현재 적용값 문구에서 동일한 표현을 재사용한다.
  return `${Number(years)}년 ${Number(months)}월 ${Number(days)}일`
}
