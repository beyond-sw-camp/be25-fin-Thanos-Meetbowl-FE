...
2026-07-02 회의록 공유 메일 본문 링크 제거와 원문 텍스트 정리

- 작업 목적: 회의록 수동 공유 메일이 링크 이동 대신 메일 본문 자체에서 요약과 원문을 함께 읽을 수 있도록 정리한다.
- 변경 파일: `src/pages/minutes/MinutesPage.vue`, `src/pages/workspace/WorkspacePage.vue`, 이 작업 기록 파일.
- 동작 변경:
  회의록 공유 메일 초안은 더 이상 `/app/minutes/{meetingId}` 링크를 포함하지 않는다.
  본문 영역은 Tiptap JSON 문자열을 그대로 싣지 않고, 텍스트 추출 유틸을 통해 사람이 읽을 수 있는 회의록 원문으로 변환해 함께 보낸다.
  원문 텍스트를 추출할 수 없는 경우에만 기존 문자열을 fallback으로 사용한다.
- 검증: `npm run build` 통과.
